...
# Set MNAR
```{r}
# ---- Data type harmonization and label adjustment ----
combined_data1 <- combined_data %>%
  # Convert all integer columns (is.integer) to double type.
  # mice, aregImpute, missRanger
  # often expect numeric or double inputs. Keeping variables as integer may
  # trigger warnings or incompatibility with internal model-fitting steps.
  mutate(across(where(is.integer), as.double)) %>%
  
  # Standardize the naming of missingness mechanisms.
  # In the simulated data, "MCAR" denotes missing completely at random,
  # while "MAR/MNAR" refers to all non-MCAR mechanisms.
  # Here, we relabel "MAR/MNAR" as simply "MNAR" in this scenario

  mutate(mcar = ifelse(mcar == "MAR/MNAR", "MNAR", mcar))
...
```{r}
# ---- Filter dataset by missingness mechanism (MNAR only) ----

combined_data2 <- combined_data1 %>%
  # Select only the subset of observations generated under the MNAR mechanism.
  # Have already done the MCAR in previous document of MCAR and MAR scenarios.
  filter(mcar == "MNAR")

# Check the resulting structure to confirm successful filtering.
...
evaluate_model <- function(data_original, method = "mice", m = 10, true_effect = 3.9) {
  ...
  # Prepare model-ready data: rename c_star to c, y_star to y
  # filter the data only inclue x y c and exclude other variables 
  # as Z and L are unobserved varables in the MNAR 
  data <- data_original %>%
    dplyr::select(y = y_star, c = c_star, x) %>%
...
# 1. complete case analysis 
  ...
  if (method == "complete") {
    ...
    if (nrow(data_cc) > 0 && length(unique(data_cc$y)) == 2) {
      fit <- try(logistf(y ~ x + c , data = data_cc), silent = TRUE)
      # try() returns "try-error" class on failure
...
# 2. mice multiple chain equation (Rubin's pooling using glm)
  ...
  if (method == "mice") {
    ...
    # Standard logistic regression is used here 
    # for compatibility with mice::pool()
    fit <- try(with(imp, glm(y ~ x + c, family = binomial())), silent = TRUE)
...
# 3. argImpute addictive adaptation model (Manual Rubin pooling)

  if (method == "aregImpute") {
    ai <- try(Hmisc::aregImpute(~ y + x + c, data = data,
    ...
    est_list <- lapply(1:m, function(i) {
      ...
      fit <- try(glm(y ~ x + c, data = df, family = binomial()), silent = TRUE)
...
# 4. missRanger (Random Forest Single Imputation)
  ...
  if (method == "missRanger") {
    ...
    fit <- try(glm(y ~ x + c, data = imputed, family = binomial()), silent = TRUE)
    if (inherits(fit, "try-error")) return(result_empty)
    sm <- coef(summary(fit))
...
# Helper: determine number of imputations by method
...
run_simulation_summary <- function(setting_row) {
  ...
  # use combined_data2, which already filters MNAR only
  # combined_data1 is used in former MCAR and MAR scenarios
  data_subset <- combined_data2 %>%
    filter(
      mcar == mechanism,
      sample_size == n,
...
write_csv(simulation_summary, "5000_MNAR_large.csv")
