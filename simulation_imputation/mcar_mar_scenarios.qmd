...
evaluate_model <- function(data_original, method = "mice", m = 10, true_effect = 3.9) {
  ...
  # Prepare model data, rename c_star to c, y_star to y
  data <- data_original %>%
    dplyr::select(y = y_star, c = c_star, x, z, l) %>%
    # covert all variables as numeric for better regression fit
...
# 1. Complete case anaysis 
  
  if (method == "complete") {
    data_cc <- data %>%
      filter(!if_any(all_of(c("y", "x", "c")), is.na)) %>%
      # only convert y to factor
    ...
    if (nrow(data_cc) > 0 && length(unique(data_cc$y)) == 2) {
      ...
      # Use try() so that model failures (due to separation or non-convergence) 
      # do not break the simulation loop.
...
# 2. mice (chain equation)
  ...
  if (method == "mice") {
    ...
    # Standard logistic regression is used
...
# 3. argImpute (Additive Regression, Bootstrapping, and Predictive Mean Matching)
